...
- 참가자 본인이 직접 **자기 기록(1라운드)** 을 입력할 수 있고, 주최자도 종목별 참가자의 라운드별 **개별 기록을 직접 입력**(종목이 Ao5면 5회, Mo3면 3회) 가능하며 자동 계산된 **평균 기록**을 확인 가능
  - 종목 추가 시 직접 입력(기타 종목)이면 **Ao5(5회 평균)/Mo3(3회 평균)** 형식을 선택 가능

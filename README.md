...
- 대회별로 **주최자·공동 주최자·스태프끼리만 볼 수 있는 대화방**이 있어 실시간으로 메시지를 주고받을 수 있음 (참가자·일반 이용자는 열람 불가, 본인 메시지는 직접 삭제 가능하고 주최자는 전체 메시지 삭제 가능)
...
- 로그인한 누구나 진행 중인 대회에 원하는 종목으로 **참가 신청** 가능하며, 이미 신청한 종목은 표시된 채로 **다른 종목을 추가로 신청**할 수 있고 **"전 종목 선택"** 버튼으로 한 번에 모두 체크할 수도 있음

...
- 대회는 **개최일 전(참가신청중) → 진행중 → 종료** 순으로 상태가 표시되며, **개최일 이전과 종료 후에는 기록을 등록할 수 없음** (종목·스크램블 등록·참가 신청은 개최일 전에도 계속 가능)
- 주최자·관리자는 **참가 신청만 마감**하거나(종목·스크램블 등록은 계속 가능), 대회를 **종료**(종목·스크램블 추가, 참가 신청, 기록 등록/수정 모두 중단)하거나 완전히 **삭제** 가능 — 대회 목록·마이페이지에도 상태가 표시됨
- 관리자는 다른 사용자를 닉네임으로 검색해 관리자로 추가 지정 가능
- 공동 주최자는 대회 목록·상세 화면에서 **주최자와 함께 표시**됨 (예: "홍길동 (공동주최: 김철수)")
- **입상 내역** 탭에서 종료된 대회 중 주최자가 순위를 지정한 종목을 모아 "대회명 - 종목명: N등" 형태로 확인 가능하며, 해당 순위가 매겨진 결승 라운드 기준 **최고기록(싱글)과 평균기록**도 함께 표시
